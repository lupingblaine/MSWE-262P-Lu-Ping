...
import java.nio.file.*;import java.util.*;import java.util.stream.*;public class Seven{
  public static void main(String[]a)throws Exception{
    var s=new HashSet<>(Arrays.asList((Files.readString(Path.of("../262/stop_words.txt"))+",abcdefghijklmnopqrstuvwxyz").split(",")));
    Arrays.stream(Files.readString(Path.of("../262/pride-and-prejudice.txt")).toLowerCase().split("[^a-z]+"))
    .filter(w->w.length()>0&&!s.contains(w))
